...
- **Layering**: `python-server/app/` splits in two. `app/api/` is the HTTP layer — `routes/`, HTTP-only response schemas in `schemas/`, the exception-to-status mapping in `errors.py`, and app assembly in `main.py`; it is the only place FastAPI is imported. `app/lib/` is the reusable layer — domain types in `types/`, use-case logic in `hierarchy.py`, SQLAlchemy tables in `models/`, SQL in `repositories/`, and the engine in `database.py`. **`app/lib/` must never import from `app/api/`.** That one-way dependency is the whole point: it is what lets a second front end (a CLI, a worker, another API) call `store_hierarchy` without dragging FastAPI along. `app/config.py` sits above both. Note `models/` means SQLAlchemy and `schemas/` means Pydantic, per FastAPI convention.
